        ...
        foreach($menu->toArray() as $group)
        {
            if(count($group['checklists']) > 0)
            {
                ...
                $group['is_updated'] = !($group['is_new'])
                    && $group_updated_at && Carbon::create($group['updated_at'])->greaterThan($group_updated_at);
                ...
                foreach($group['checklists'] as &$checklist)
                {
                    ...
                    $checklist['is_updated'] = !($group['is_new']) && !($group['is_updated'])
                        && $checklist_updated_at && !($checklist['is_new']) && Carbon::create($checklist['updated_at'])->greaterThan($checklist_updated_at);;
